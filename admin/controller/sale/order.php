...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['prepareshipment'] = $this->url->link('sale/order/prepareshipment', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
			$data['updateshipment'] = $this->url->link('sale/order/updateshipment', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
			$data['confirmshipment'] = $this->url->link('sale/order/confirmshipment', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
			...
			if($this->model_extension_shipping_flagship->isFlagshipInstalled()){
				$data['flagship'] = 1;
				$flagship_shipment_id = $this->getFlagshipShipmentId();
				$data['flagship_shipment_id'] = $flagship_shipment_id == NULL ? 0 : $flagship_shipment_id;
				$data['shipment_confirmed'] = $flagship_shipment_id != NULL ? $this->isFlagshipShipmentConfirmed($flagship_shipment_id) : 0;
			}
	...
	public function updateShipment() : int {
		...
		try{
			$shipment = $flagship->editShipmentRequest($payload,$flagship_shipment_id)->execute();
	...
	public function confirmShipment() : int {
		...
		$data['footer'] = $this->load->controller('common/footer');
		$flagship_shipment_id = $this->getFlagshipShipmentId();
		$data['flagship_url'] = $this->config->get('smartship_web_url').'/shipping/'.$flagship_shipment_id.'/convert';
		...
		$this->response->setOutput($this->load->view('sale/order_flagship_confirm_shipment', $data));
		return 0;
	...
	public function prepareshipment() : int {
		...
		$flagship = new Flagship($this->config->get('shipping_flagship_token'), $this->config->get('smartship_api_url'), 'OpenCart', '1.0.0');
		try{
			$shipment = $flagship->prepareShipmentRequest($payload)->execute();

	        $this->load->model('extension/shipping/flagship');
	        $this->model_extension_shipping_flagship->updateFlagshipShipmentId($shipment->getId(),$order_id);
	        $data['flagship_shipment_id'] = $shipment->getId();
	        $this->response->redirect($this->url->link('sale/order/info', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true));
	...
	protected function isFlagshipShipmentConfirmed(int $flagship_shipment_id) : int {
		$flagship = new Flagship($this->config->get('shipping_flagship_token'), $this->config->get('smartship_api_url'), 'OpenCart', '1.0.0');
		$shipment = $flagship->getShipmentByIdRequest($flagship_shipment_id)->execute();
		$status = $shipment->getStatus();

		if(strcasecmp('prequoted', $status) !== 0){
			return 1;
		}
		return 0;
	}
	...
	protected function getPayload(int $order_id) : array {
		...
		$to = [
			"name" => $toName,
      		...
      		"address" => $order_info["shipping_address_1"] == NULL ? $order_info["payment_address_1"] : $order_info["shipping_address_1"],
